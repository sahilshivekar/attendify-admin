...
class UpdateStaffDetailsUseCase @Inject constructor(
    private val staffRepository: StaffRepository
) {
    operator fun invoke(requestBody: UpdateStaffDetailsRequest): Flow<Resource<Staff?>> = flow {
        emit(Resource.Loading())

        val response = runCatching {
            staffRepository.updateStaffDetails(requestBody)
        }
