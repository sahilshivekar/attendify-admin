...
class UpdateStudentImageUseCase @Inject constructor(
    private val studentRepository: StudentRepository
) {
    operator fun invoke(studentId: Int, studentImageFile: File): Flow<Resource<Student?>> = flow {
        emit(Resource.Loading())

        val response = runCatching {
            studentRepository.updateStudentImage(studentId, studentImageFile)
        }
