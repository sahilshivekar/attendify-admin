...
class GetTeachingSubjectsUseCase @Inject constructor(
    private val staffRepository: StaffRepository
) {
    operator fun invoke(staffId: Int): Flow<Resource<List<TeacherTeaches>?>> = flow {

        emit(Resource.Loading())

        val response = runCatching {
            staffRepository.getTeachingSubjects(staffId)
        }

        response.onSuccess { response ->
            if (response.isSuccessful) {
                emit(Resource.Success(response.body()?.data?.map{it.toTeacherTeaches()}))
            } else {
