...
class RemoveCourseFromBranchWithSemesterNumberUseCase @Inject constructor(
    private val courseRepository: CourseRepository
) {
    operator fun invoke(
        requestBody: RemoveCourseFromBranchWithSemesterNumberRequest
    ): Flow<Resource<String?>> = flow {

        emit(Resource.Loading<String?>())

        val response = runCatching {
            courseRepository.removeCourseFromBranchWithSemesterNumber(requestBody)
        }
