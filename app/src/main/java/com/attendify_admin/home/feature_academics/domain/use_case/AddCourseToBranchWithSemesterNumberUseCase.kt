...
class AddCourseToBranchWithSemesterNumberUseCase @Inject constructor(
    private val courseRepository: CourseRepository
) {
    operator fun invoke(requestBody: AddCourseToBranchWithSemesterNumberRequest): Flow<Resource<Course>> = flow {

        emit(Resource.Loading<Course>())

        val response = runCatching {
            courseRepository.addCourseToBranchWithSemesterNumber(requestBody)
        }
