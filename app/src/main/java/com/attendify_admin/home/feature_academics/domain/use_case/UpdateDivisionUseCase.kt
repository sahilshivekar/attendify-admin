...
class UpdateDivisionUseCase @Inject constructor(
    private val divisionRepository: DivisionRepository
) {
    operator fun invoke(requestBody: UpdateDivisionRequest): Flow<Resource<Division?>> = flow {

        emit(Resource.Loading())

        val response = runCatching {
            divisionRepository.updateDivision(requestBody)
        }
