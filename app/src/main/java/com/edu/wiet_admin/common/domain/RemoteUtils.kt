...
object RemoteUtils {
    ...
    private fun <T> getErrorMessage(response: Response<T>): String? {
        ...
        errorBody?.let {
            val errorMessage = Gson().fromJson(errorBody, WietApiResponse::class.java).message
            return errorMessage
        }
        return null
    }
    ...
    fun <T> responseFlow(
        apiCall: suspend () -> Response<WietApiResponse<T>>
    ): Flow<Resource<WietApiResponse<T>>> = flow {
        try {
