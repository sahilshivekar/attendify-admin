package com.edu.wiet_admin.admin_auth.data.remote
...
interface AuthApi {
    ...
    suspend fun login(@Body requestBody: LoginRequest): Response<WietApiResponse<LoginData?>>

    @POST("api/v1/admin/forgot-password")
    suspend fun forgotPassword(@Body requestBody: ForgotPasswordRequest): Response<WietApiResponse<VerificationCodeData?>> // forgot password and email verification have same response on success
    ...
    suspend fun verifyCode(@Body requestBody: VerifyCodeRequest): Response<WietApiResponse<GetAccessRefreshTokenData?>>
