    ...
    init {
        ...
        getSchemesUseCase(searchQuery = "").onEach { result ->
            when (result) {
                ...
                is Resource.Success -> {
                    state.value = state.value.copy(
                        schemeOptions = result.data?.data ?: emptyList()
                    )
                }
            }
    ...
    fun onEvent(event: AddStudentEvent) {
        when (event) {
            ...
            is AddStudentEvent.MiddleNameChanged -> {
                state.value = state.value.copy(middleName = event.newMiddleName, isMiddleNameError = null)
            }
            ...
            is AddStudentEvent.AdmissionYearChanged -> {
                state.value = state.value.copy(admissionYear = event.newAdmissionYear, isAdmissionYearError = null)
            }

            is AddStudentEvent.AdmissionTypeChanged -> {
                state.value = state.value.copy(admissionType = event.newAdmissionType, isAdmissionTypeError = null)
            }

            is AddStudentEvent.BranchChanged -> {
                state.value = state.value.copy(selectedBranch = event.newBranch, isBranchError = null)
            }

            is AddStudentEvent.SchemeChanged -> {
                state.value = state.value.copy(selectedScheme = event.newScheme, isSchemeError = null)
            }

            is AddStudentEvent.AcademicStatusChanged -> {
                state.value = state.value.copy(academicStatus = event.newAcademicStatus, isAcademicStatusError = null)
            }
            ...
            is AddStudentEvent.StudentImageUriUpdated -> {
                if (event.updatedUri == null) {
                    ...
                    viewModelScope.launch {
                        val file: File? = UriToFileUtil.getFileFromUri(
                            context,
                            event.updatedUri
                        )
                        val fileName = UriToFileUtil.getFileNameFromUri(
                            context,
                            event.updatedUri
                        )
                        state.value = state.value.copy(
    ...
    private fun isStudentDataValid(): Boolean {
        return when {
            ...
            state.value.admissionYear.isBlank() -> {
                state.value = state.value.copy(isAdmissionYearError = "Admission year cannot be empty")
                false
            }

            state.value.admissionType.isBlank() -> {
                state.value = state.value.copy(isAdmissionTypeError = "Admission type cannot be empty")
                false
            }
