...
interface SchemeApi {
    ...
    suspend fun getSchemes(@Query("searchQuery") searchQuery: String?): Response<PresencifyApiResponse<List<SchemeDto>?>>

    @POST("api/v1/scheme/admin/add")
    suspend fun addScheme(@Body requestBody: AddSchemeRequest): Response<PresencifyApiResponse<SchemeDto?>>

    @PUT("api/v1/scheme/admin/update")
    suspend fun updateScheme(@Body requestBody: UpdateSchemeRequest): Response<PresencifyApiResponse<SchemeDto?>>

    @DELETE("api/v1/scheme/admin/remove")
    suspend fun removeScheme(@Query("id") schemeId: Int): Response<PresencifyApiResponse<String?>>

    @GET("api/v1/scheme/admin/get-scheme-by-id")
    suspend fun getSchemeById(@Query("schemeId") schemeId: Int): Response<PresencifyApiResponse<SchemeDto?>>
}