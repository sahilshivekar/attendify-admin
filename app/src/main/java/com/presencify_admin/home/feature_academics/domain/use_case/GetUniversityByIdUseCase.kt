...
class GetUniversityByIdUseCase @Inject constructor(
    private val universityRepository: UniversityRepository
) {
    operator fun invoke(universityId: Int): Flow<Resource<University?>> = flow {

        emit(Resource.Loading<University?>())

        val response = runCatching {
            universityRepository.getUniversityById(universityId)
        }
