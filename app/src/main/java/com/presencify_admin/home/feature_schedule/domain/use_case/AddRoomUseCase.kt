...
class AddRoomUseCase @Inject constructor(
    ...
    operator fun invoke(requestBody: AddRoomRequest): Flow<Resource<Room?>> = flow {

        emit(Resource.Loading())

        val response = runCatching {
            roomRepository.addRoom(requestBody)
        }

        response.onSuccess { response ->
            if (response.isSuccessful) {
                emit(Resource.Success(response.body()?.data?.toRoom()))
            } else {
