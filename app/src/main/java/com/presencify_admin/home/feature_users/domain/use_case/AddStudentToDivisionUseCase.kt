...
import com.presencify_admin.common.data.remote.dto.response.toStudentDivision
...
class AddStudentToDivisionUseCase @Inject constructor(
    private val studentRepository: StudentRepository
) {
    operator fun invoke(
        requestBody: AddStudentToDivisionRequest
    ): Flow<Resource<StudentDivision?>> = flow {

        emit(Resource.Loading<StudentDivision?>())

        val response = runCatching {
            studentRepository.addStudentToDivision(requestBody)
        }

        response.onSuccess { response ->
            if (response.isSuccessful) {
                emit(Resource.Success(response.body()?.data?.toStudentDivision()))
            } else {
