...
import com.presencify_admin.home.feature_academics.domain.use_case.GetAllDivisionsUseCase
...
import com.presencify_admin.home.feature_users.domain.use_case.AddStudentToDivisionUseCase
...
class AssignStudentDivisionViewModel @Inject constructor(
    ...
    private val addStudentToDivisionUseCase: AddStudentToDivisionUseCase,
    private val getAllDivisionsUseCase: GetAllDivisionsUseCase,

    ) : ViewModel() {
    private val _state = MutableStateFlow(AssignStudentDivisionState())
...
//                                studentYear = if (
//                                    (student.studentDivisions?.size ?: 0) > 0 &&
//                                    student.studentDivisions?.first()?.division?.semesterNumber != null
//                                )
//                                    StudentUtils.getCurrentYearFromSem(
//                                        student.studentDivisions.first().division.semesterNumber
//                                    ) else null,
    ...
    private fun assignStudentsToDivision() {
        val divisionId = _state.value.selectedDivision?.id
        divisionId?.let {
            _state.value.selectedStudentIds.forEach { studentId ->
                addStudentToDivisionUseCase(
                    requestBody = AddStudentToDivisionRequest(
                        studentId = studentId,
                    ...
                    when (result) {
                        ...
                        is Resource.Success -> {
                            result.data?.let { studentDivision ->
                                var student = _state.value.selectedStudents.find { student ->
                                    student.id == studentId
                                }
                                student?.let {
...
//                            )
    ...
    fun onEvent(event: AssignStudentDivisionEvent) {
        ...
        when (event) {
            ...
            is AssignStudentDivisionEvent.FetchMatchingDivisions -> {
                val division = _state.value.selectedSemesterNumber
                val academicYear = _state.value.selectedAcademicYear
                val branchId = _state.value.selectedBranch?.id

                if (division == null) {
                    viewModelScope.launch {
                        SnackbarController.sendEvent(SnackbarEvent(message = "Please select a division number"))
                    }
                    return
                }
                            ...
                            is Resource.Success -> {
                                _state.update {
                                    it.copy(
                                        ...
                                        foundDivisions = result.data ?: persistentListOf(),
                                    )
                                }
                                result.data?.let {
                                    if (it.isEmpty()) {
                                        SnackbarController.sendEvent(
                                            SnackbarEvent(
                                                message = "No division found for selected division details"
                                            )
                                        )
                                    }
                                }
            ...
            is AssignStudentDivisionEvent.AcademicYearDropdownVisibilityChanged -> {
                _state.update {
                    it.copy(
            ...
            is AssignStudentDivisionEvent.BranchDropdownVisibilityChanged -> {
                _state.update { it.copy(isBranchDropdownExpanded = event.expanded) }
            }

            is AssignStudentDivisionEvent.SemesterDropdownVisibilityChanged -> {
                _state.update { it.copy(isSemesterDropdownExpanded = event.expanded) }
            }

            is AssignStudentDivisionEvent.AcademicYearSelected -> {
                _state.update {
                    it.copy(
            ...
            is AssignStudentDivisionEvent.BranchSelected -> {
                _state.update {
                    it.copy(
            ...
            is AssignStudentDivisionEvent.DivisionSelected -> {
                _state.update { it.copy(selectedDivision = event.division) }
            }

            is AssignStudentDivisionEvent.SemesterNumberSelected -> {
                _state.update { it.copy(selectedSemesterNumber = event.semesterNumber) }
            }

            AssignStudentDivisionEvent.AssignStudentsClicked -> {
                assignStudentsToDivision()
            }

            is AssignStudentDivisionEvent.RemoveStudentClicked -> {
