...
class RemoveTeachingSubjectUseCase @Inject constructor(
    private val staffRepository: StaffRepository
) {
    operator fun invoke(teacherSubjectId: Int): Flow<Resource<Unit?>> = flow {
        emit(Resource.Loading())

        val response = runCatching {
            staffRepository.removeTeachingSubject(teacherSubjectId)
        }
