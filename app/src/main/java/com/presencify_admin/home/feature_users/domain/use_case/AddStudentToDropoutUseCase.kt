...
class AddStudentToDropoutUseCase @Inject constructor(
    private val studentRepository: StudentRepository
) {
    operator fun invoke(
        requestBody: AddDropoutRequest
    ): Flow<Resource<Dropout?>> = flow {

        emit(Resource.Loading<Dropout?>())

        val response = runCatching {
            studentRepository.addStudentToDropout(requestBody)
        }

        response.onSuccess { response ->
            if (response.isSuccessful) {
                emit(Resource.Success(response.body()?.data?.toDropout()))
            } else {
