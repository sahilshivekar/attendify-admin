package com.presencify_admin.common.domain
...
object RemoteUtils {
    ...
    fun <T> getErrorMessage(response: Response<T>): String? {
        ...
        errorBody?.let {
            val errorMessage = Gson().fromJson(errorBody, PresencifyApiResponse::class.java).message
            return errorMessage
        }
        return null
    }
    ...
    fun <T> responseFlow(
        apiCall: suspend () -> Response<PresencifyApiResponse<T>>,
    ): Flow<Resource<PresencifyApiResponse<T>>> = flow {
        try {
